...
def get_keyboard_language():
    ...
    if language_id_hex in languages.keys():
        return languages[language_id_hex] + ' ' + str (language_id_hex)
    else:
        # Return language id hexadecimal value if not found.
        return str(language_id_hex)

def set_keyboard_language(language_id):
    language_id_hex = hex(language_id)
    user32 = ctypes.WinDLL('user32', use_last_error=True)

    # Get the current active window handle
    handle = user32.GetForegroundWindow()
    result = win32api.SendMessage(handle, WM_INPUTLANGCHANGEREQUEST, 0, language_id)
    if result == 0:
        return 'Set the  keyboard successfully! ' + language_id_hex
    else:
        return 'Set the  keyboard fail ... ' + language_id_hex



'''
print(get_keyboard_language()) # read the keyboard language
print(set_keyboard_language(0x040c)) # set the keyboard language french
print(get_keyboard_language()) # read the keyboard language
'''